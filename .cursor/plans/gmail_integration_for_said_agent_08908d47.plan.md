...
Интеграция позволит Саиду обрабатывать входящие письма как полноценному менеджеру. Мы будем использовать существующий `api` для «мозгов» и добавим новый коннектор для почты.
...
3. Для каждого письма:
    ...
    - Запрос к `api` с метаданными `channel: email`.
