...
async def _fetch_products_from_1c(product_codes: List[str]) -> Dict[str, dict]:
    ...
    """
    from .c1_schemas import parse_get_detailed_items_payload
...
class OrderInfo(BaseModel):
    ...
    # Backward compatible: старое текстовое описание (оставляем как fallback)
    product_details: Optional[str] = Field(
        None,
        description="Текстом: что заказывает клиент (название/объем/размеры). Можно использовать как fallback",
    )
...
class ManagerHandover(BaseModel):
    # Backward compatible short summary (оставляем)
...
async def collect_order_info(order: OrderInfo) -> str:
    ...
    fields = {
        ...
        "Детали заказа (fallback)": order.product_details or "",
    ...
    message["To"] = sales_email
    message["Subject"] = subject
    message.set_content(f"Новый заказ: {order.client_name} - {main_product}") # Plain text fallback
    message.add_alternative(html_body, subtype='html')
