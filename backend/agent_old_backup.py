...
# Импортируем tools
from tools import search_company_info, call_manager, collect_order_info, search_products_tool, get_product_live_details, calculate
from params_manager import ParamsManager
from runtime_config import get_secret_cached
...
def get_llm_config():
    ...
    api_key = get_secret_cached("openrouter_token")
    if not api_key:
...
def build_agent_prompt(req: ModelRequest) -> str:
    ...
    kb_text = params_manager.get_knowledge_base_for_prompt()  # Компактный формат (только справочник разделов)

    system_prompt = (
        prompt_base
        + f"\n\nКОНТЕКСТ КЛИЕНТА: {user_name} ({user_phone}), канал: {channel_info}. Сейчас {current_time_str}, {weekday_ru}."
