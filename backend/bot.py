...
import httpx
from schemas.service_schemas import MessageResponse, MessageRequest
...
API_URL = getenv("API_URL", "http://localhost:5537")
...
async def ai_request(
    ...
    try:
        ...
        logger.error(f"HTTP error when calling AI service for user {user_id}: {e.response.status_code} - {e}")
        if e.response.status_code == 429:
            return MessageResponse(response="Извините, превышен лимит запросов к AI. Попробуйте через минуту.")
        return MessageResponse(response="Извините, сервис временно недоступен. Попробуйте позже.")
    except httpx.TimeoutException as e:
        logger.error(f"Timeout when calling AI service for user {user_id}: {e}")
        return MessageResponse(response="Извините, запрос занял слишком много времени. Попробуйте упростить вопрос.")
    except Exception as e:
...
async def handle_message(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        # Отправляем ответ пользователю (конвертируем markdown в HTML)
        formatted_response = telegram_format(response.response)
...
async def on_startup():
    ...
    if await check_api_health():
        logger.info(f"AI service is available at {API_URL}")
    else:
        logger.warning(f"AI service is not available at {API_URL}. Bot will still start but may fail.")
