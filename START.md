...
# Запуск сервиса
uv run uvicorn ai_service:app --host 0.0.0.0 --port 5537 --reload
```
...
3. CORS настроен в `backend/ai_service.py`

### Проблема: Ошибка авторизации

**Решение**: 
1. Проверьте, что `JWT_SECRET` установлен в `.env`
2. Пересоздайте админа: `docker-compose exec ai_service python -m db.create_admin`
...
```bash
# Инициализация БД
docker-compose exec ai_service python -m db.init_db

# Создание админа
docker-compose exec ai_service python -m db.create_admin
```
